...
import simulation.SimulatedBank.Failure;
import simulation.SimulatedBank.Success;
...
public class Message
{
	...
	/**
	 * Simulate initiation of a deposit. At this point, the bank only approves the validity of the deposit - no changes to the records are made until the envelope is actually inserted 
	 * @return  status code derived from current values
	 */
	public Status initiateDeposit() {
		int cardNumber = getCard().getNumber();
		int accountNumber = SimulatedBank.ACCOUNT_NUMBER[cardNumber][getToAccount()];
		if (accountNumber == 0)
